...
- **Paths agents must not write**: `archilles/**`, `schema/**`, `.github/**`. A design change is a proposal on a `design/*` branch for James to approve, never an edit made in passing to make a check go green. If `archilles diff` fails, fix the code — do not edit the design to permit the violation. **This is currently an instruction, not a guarantee:** push-time path restriction needs an org-owned repo and GitHub rejects it on a personal one, so `CODEOWNERS` (review-time) is the only live guard. Treat the restriction as binding anyway. See `docs/GOVERNANCE.md`.
- `CODEOWNERS`: `archilles/**`, `schema/**`, `.github/**` → James.
- **Agents act as GitHub Apps, not as James.** One App per role — `archilles-engineer`, `-architect`, `-orchestrator`, `-bot`, `-release` — with role instructions in `.claude/agents/` and manifests in `.github/app-manifests/`. Anything touching GitHub goes through `scripts/as-app.sh <app> <command>`, which mints a scoped 1-hour installation token and sets the bot author so the commit is attributed to `<app>[bot]`. This is agent-driven, not CI: there is no workflow that runs these roles.
- Rulesets are code in `.github/rulesets/`, applied by `scripts/apply-rulesets.sh`. `docs/GOVERNANCE.md` is this repo's instantiation; `.claude/skills/archilles-governance/` is the reusable form other projects adopt.
- An agent's tool list must agree with its App's permissions. The orchestrator has `contents: read` and therefore no `Edit`/`Write` tool — it coordinates, it does not commit. Where the two disagree, the weaker is the real policy.
