...
- **Paths agents must not write**: `archilles/**`, `schema/**`, `.github/**` — enforced at push by a ruleset, bypassed only by `archilles-architect` and admins. A design change is a proposal on a `design/*` branch for James to approve, never an edit made in passing to make a check go green. If `archilles diff` fails, fix the code — do not edit the design to permit the violation.
