...
- Agent identities are one GitHub App per role (`archilles-engineer`, `-architect`, `-orchestrator`, `-bot`, `-release`), manifests in `.github/app-manifests/`. Rulesets are code in `.github/rulesets/`, applied by workflow. `docs/GOVERNANCE.md` has the permission matrix and setup path.
